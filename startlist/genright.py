...
class GenRight:
    ...
    def _generate_wave_header(self, event, header1=None, header2=None):
        ...
        #    self.text(header1)
        if header1:
            with self.tag('tr', klass='part-tr-15',):
    ...
    def _generate_wave_row(self, event, event_id, wave_name, participant):
        ...
        #with self.tag('tr', klass='part-tr fs-s', id=wave_table_row_id,
        with self.tag('tr', klass='part-tr-s', id=wave_table_row_id,
        ...
        #with self.tag('tr', klass='part-note fs-s', id=f"{wave_table_note_id}", ):
        with self.tag('tr', klass='part-note', id=f"{wave_table_note_id}", ):
            #with self.tag('td', klass='thtd', colspan=6, style='text-center: left; padding: 1px;', ):
            with self.tag('td', klass='thtd', colspan=6, ):
                with self.tag('div', klass='div-note', ):
                    ...
                    with self.tag('input', type='text', klass='note-input', id=inputFieldId,
                                  oninput=f"HNKD(event, '{bib}', '{otherFieldId}', )",): pass 
    ...
    def generate_right(self):
        ...
        with self.tag('div', klass='right'):
            ...
            for event_id, event_info in self.parent.data.items():
                ...
                with self.tag('table', klass="table tablesorter part-table", id=wave_table_all_id, 
            ...
            for event_id, event_info in self.parent.data.items():
                ...
                for i, (wave_name, wave_data) in enumerate(event_info['waves'].items()):
                    ...
                    with self.tag('table', klass="table tablesorter part-table", id=wave_table_id, 
                                  style="display:none; padding: 1px;"):
                        with self.tag('thead', klass='part-thead',
                                      style='width:100%;'):
